...
export default function StockModal({ open, lobbyId, snapshot, stock, onClose }: Props) {
  ...
  const [enforceMinPool, setEnforceMinPool] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    if (stock) setEnforceMinPool((stock as any).enforce_min_pool ?? false);
  }, [stock]);
  ...
  const price = Math.max(1, stock?.price || P || 1); // for display only; equals P
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000 }} onClick={onClose}>
      <div className="card" style={{ background: '#fff', width: 'min(720px, 92vw)', borderRadius: 10, padding: 14, boxShadow: '0 10px 30px rgba(0,0,0,0.2)' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="ui-sm" style={{ display: 'grid', gap: 6, maxHeight: 220, overflowY: 'auto', marginTop: 6 }}>
              {stock.holdings.length === 0 ? <div style={{ opacity: 0.7 }}>No outside investors yet</div> : stock.holdings.map((h) => (
                <div key={h.investor} style={{ display: 'flex', justifyContent: 'space-between' }}>
                  ...
                  <span>{((h.percent ?? 0) * 100).toFixed(2)}% ({h.shares} sh)</span>
                </div>
              ))}
              <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: '1px dashed #eee', paddingTop: 6 }}>
                <strong>Owner</strong>
                <span>{((stock.owner_percent ?? Math.max(0, 1 - stock.holdings.reduce((a, b) => a + (b.percent || 0), 0))) * 100).toFixed(2)}%</span>
              </div>
            </div>
          </div>
          <div className="ui-labelframe">
            ...
            <div className="ui-sm" style={{ display: 'grid', gap: 6, marginTop: 6 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between' }}><span>Holdings</span><strong>{(myHolding.percent * 100).toFixed(2)}% ({myHolding.shares} sh)</strong></div>
            </div>
    {myName === stock.owner ? (
              <div className="ui-labelframe" style={{ marginTop: 10 }}>
                ...
                <div className="ui-sm" style={{ display: 'grid', gap: 8, alignItems: 'center', marginTop: 6 }}>
                  ...
                  <label style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <input type="checkbox" checked={enforceMinPool} onChange={(e) => setEnforceMinPool(e.target.checked)} /> Enforce Minimum Pool to Accept Invest
                    <span>Total $<input type="number" min={0} value={minPoolTotal} onChange={(e) => setMinPoolTotal(parseInt(e.target.value || '0', 10))} style={{ width: 120, marginLeft: 4 }} /></span>
                    <span>Owner $<input type="number" min={0} value={minPoolOwner} onChange={(e) => setMinPoolOwner(parseInt(e.target.value || '0', 10))} style={{ width: 120, marginLeft: 4 }} /></span>
                  </label>
                  <div>
                    <button className="btn btn-ghost" onClick={() => { s.emit('game_action', { id: lobbyId, action: { type: 'stock_settings', owner: stock.owner, allow_investing: allowInvesting, enforce_min_buy: enforceMinBuy, min_buy: Math.max(0, Math.floor(minBuy || 0)), enforce_min_pool: enforceMinPool, min_pool_total: Math.max(0, Math.floor(minPoolTotal || 0)), min_pool_owner: Math.max(0, Math.floor(minPoolOwner || 0)) } }); }}>Save</button>
                  </div>
                </div>
              </div>
            ) : null}
        ...
        {tab === 'buy' ? (
          <div className="ui-labelframe" style={{ marginTop: 8 }}>
            ...
            <div className="ui-sm" style={{ marginTop: 6, opacity: 0.9 }}>
              You’ll invest <strong>${A}</strong>. New ownership: <strong>{(newMyPercentBuy * 100).toFixed(2)}%</strong>
            </div>
            <div style={{ marginTop: 8 }}>
              <button className="btn" onClick={invest} disabled={A <= 0 || A > myCash || (((stock as any).enforce_min_buy ?? false) && A < (stock.min_buy || 0)) || (((stock as any).allow_investing === false)) || (myName === stock.owner)}>Confirm Buy</button>
              {myName === stock.owner && <span className="ui-sm" style={{ marginLeft: 8, color: '#c0392b' }}>Owners can’t invest in their own stock</span>}
          ...
          <div className="ui-labelframe" style={{ marginTop: 8 }}>
            ...
            <div className="ui-sm" style={{ marginTop: 4, opacity: 0.9 }}>
              New ownership: <strong>{(newMyPercentSell * 100).toFixed(2)}%</strong>
            </div>
            <div style={{ marginTop: 8 }}>
